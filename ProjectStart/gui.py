...
import subprocess
import requests
from ProjectStart.ReadConfig import ReadConfig
...
class GUI:
    def __init__(self, master):
        ...
        self.stop_button = tk.Button(master, text="Stop", command=self.stop, font=("Helvetica", 20), bg="#f44336",
                                     fg="white", padx=30, pady=15, state=tk.DISABLED)
    ...
    def start(self):
        ...
        subprocess.run(["bash", "robot_start.sh"])
        config = ReadConfig()
    ...
    def stop(self):
        ...
        subprocess.run(["bash", "robot_stop.sh"])
        config = ReadConfig()
        ip = config.read_data("IP_RASPBERRY")
