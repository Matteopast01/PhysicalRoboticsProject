class PriorityQueue:
    def __init__(self, arr=None, has_higher_priority=lambda x, y: x[0] < y[0], id_of=lambda x: x[1]):
        if arr is None:
            arr = []
        self._heap = arr
    ...
    def update_priority(self, new_priority, elem_id):
        idx = self._get_elem_idxs(elem_id=elem_id)
        if idx is None:
            return

        current_priority, elem = self[idx]
